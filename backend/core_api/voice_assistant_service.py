...
# Load from parent directory (backend/.env)
...
class BasicVoiceAssistant:
    ...
    async def start(self):
        ...
        try:
            ...
            self._update_status("connecting", "Connecting to VoiceLive API...")
            ...
            async with connect(
                ...
                # Process events
                # Run _process_events until _shutdown_event is set
             ...
             logger.info("Voice assistant session cancelled")
        finally:
            if self.audio_processor:
                self.audio_processor.shutdown()
            self.session_ready = False
            self._update_status("stopped", "Voice assistant stopped.")
    ...
    async def _handle_event(self, event):
        ...
        if event.type == ServerEventType.SESSION_UPDATED:
            ...
            self._update_status("listening", "Hearing you...")
            print("🎤 Listening...")
            ...
            self._update_status("processing", "Thinking...")
            print("🤔 Processing...")
            ...
            self._update_status("listening", "Ready...")
            ...
            self._response_api_done = True
             ...
             # Just in case we get text deltas (unlikely with audio modality usually for pure audio, but useful if text is enabled)
             pass

        elif event.type == ServerEventType.CONVERSATION_ITEM_CREATED:
             logger.debug("Conversation item created: %s", event.item.id)
             # Try to extract text content if available to show in UI
             # Note: Structure depends on exact SDK version models but usually related to message content
             if hasattr(event.item, 'content') and event.item.content:
                  for content_part in event.item.content:
                       if hasattr(content_part, 'text') and content_part.text:
                            # It's a text part
                            if event.item.role == "assistant":
                                 self._update_status("speaking", content_part.text)
                            elif event.item.role == "user":
                                 self._update_status("user_input", content_part.text)
                       elif hasattr(content_part, 'transcript') and content_part.transcript:
                             # Sometimes comes as transcript for audio
                             if event.item.role == "assistant":
                                 self._update_status("speaking", content_part.transcript)
            ...
            if "Cancellation failed: no active response" in msg:
                ...
                self._update_status("error", f"Error: {msg}")
                print(f"Error: {msg}")
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default=os.environ.get("AZURE_VOICELIVE_ENDPOINT", "https://anurag6569201-1049-resource.services.ai.azure.com/"),
    )
